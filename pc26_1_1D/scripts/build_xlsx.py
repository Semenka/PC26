...
(spec verbatim from task brief §XLSX SCRIPT BLOCK)
"""
...
OUT = "/home/user/pc26_1_1D/deliverables/PC26_1.1D_Debottlenecking_Opportunities.xlsx"

NAVY = "0A2540"; DEEP = "1E3A5F"; TEAL = "0891B2"; MINT = "34D399"; OIL = "F59E0B"
WATER = "06B6D4"; BAD = "DC2626"; WARN = "F59E0B"; GOOD = "16A34A"
CREAM = "FFFFFF"; LIGHT = "F8FAFC"; LINE_GRY = "CBD5E1"; INK = "0F172A"; DIM = "64748B"
...
ws.merge_cells("A1:H1"); ws["A1"] = "PC26 — Obj 1.1D — Shah Field De-Bottlenecking Opportunities"
ws["A1"].font = BIG_TITLE; ws.row_dimensions[1].height = 30
ws.merge_cells("A2:H2"); ws["A2"] = "Quantified opportunity catalogue — surface & subsurface"; ws["A2"].font = ITAL
ws.merge_cells("A3:H3"); ws["A3"] = "Issued 19-May-2026 · TotalEnergies ALSG × ADNOC Onshore"; ws["A3"].font = SMALL
...
for i, (k, v) in enumerate(contents, start=6):
    ...
    ws.cell(row=i, column=2, value=v).font = NORM; ws.cell(row=i, column=2).alignment = LFT
...
for i, opp in enumerate(ranked, start=5):
    rank = i - 4
    vals = [rank, opp[0], opp[1], opp[2], opp[3], opp[4], opp[5], opp[6], opp[7]]
    for j, v in enumerate(vals, start=1):
        c = ws.cell(row=i, column=j, value=v)
        c.font = BOLD if j in (1,2) else NORM; c.border = BORDER
        if i % 2 == 0: c.fill = ALT_FILL
        if j in (5,6,7,8): c.alignment = RGT
tot_row = 5 + len(ranked)
ws.cell(row=tot_row, column=3, value="TOTAL realistic envelope (with 25 % overlap deduction)").font = BOLD
ws.cell(row=tot_row, column=5, value=f"=ROUND(SUM(E5:E{tot_row-1})*0.75,0)")
ws.cell(row=tot_row, column=6, value=f"=ROUND(SUM(F5:F{tot_row-1})*0.75,0)")
ws.cell(row=tot_row, column=7, value=f"=ROUND(SUM(G5:G{tot_row-1})*0.75,0)")
ws.cell(row=tot_row, column=8, value=f"=ROUND(SUM(H5:H{tot_row-1})*0.75,0)")
for j in range(1, 10):
...
chart.y_axis.title = "Opportunity"; chart.x_axis.title = "Gain (BOPD)"
...
chart.height = 12; chart.width = 22; chart.legend = None
...
for i, (k, a, b, cmt) in enumerate(oil_rows, start=5):
    ws.cell(row=i, column=1, value=k).font = BOLD
    if a is not None:
        ws.cell(row=i, column=2, value=a).font = NORM; ws.cell(row=i, column=2).alignment = RGT
    if b is not None:
        ws.cell(row=i, column=3, value=b).font = NORM; ws.cell(row=i, column=3).alignment = RGT
...
chart = BarChart(); chart.type = "col"; chart.style = 12
chart.title = "Shah field oil — Allowable vs TR vs Actual (BOPD)"; chart.y_axis.title = "BOPD"
...
for i, row in enumerate(BOTTLENECK_MAP, start=5):
    for j, v in enumerate(row, start=1):
        ...
        if i % 2 == 0: c.fill = ALT_FILL
        if j == 7:
            color_map = {"Subsurface": TEAL, "Surface": OIL, "Surface/Completion": OIL,
                         "Completion": DEEP, "Water Injection": WATER}
            if v in color_map:
                c.font = Font(name="Arial", size=10, bold=True, color=color_map[v])

chart = BarChart(); chart.type = "bar"; chart.style = 13
chart.title = "BOPD lost by bottleneck category"; chart.x_axis.title = "BOPD lost vs SH cap"
...
for i, opp in enumerate(OPPORTUNITIES, start=5):
    ...
    for j, v in enumerate(vals, start=1):
        c = ws.cell(row=i, column=j, value=v)
        c.font = BOLD if j in (1, 3) else NORM; c.border = BORDER
        if j >= 4 and j != 9: c.alignment = RGT
        if i % 2 == 0: c.fill = ALT_FILL

chart = BarChart(); chart.type = "bar"; chart.grouping = "stacked"
chart.overlap = 100; chart.style = 11
chart.title = "2026 gain range per opportunity (low → high, BOPD)"; chart.x_axis.title = "BOPD"
sd = wb["_chart_data"]
...
chart.add_data(data1, titles_from_data=True); chart.add_data(data2, titles_from_data=True)
chart.set_categories(cats); chart.height = 14; chart.width = 24
...
wf = [("Start", "Current actual production", 0, 75733)]
cum = 75733
...
for i, (step, item, inc, cumv) in enumerate(wf, start=5):
    ...
    ws.cell(row=i, column=3, value=inc); ws.cell(row=i, column=3).alignment = RGT
    ws.cell(row=i, column=4, value=cumv).font = BOLD; ws.cell(row=i, column=4).alignment = RGT
    for j in range(1, 5):
        ws.cell(row=i, column=j).border = BORDER
    if step == "Start":
        for j in range(1, 5):
            ws.cell(row=i, column=j).fill = NAVY_FILL
            ws.cell(row=i, column=j).font = WHITE_BOLD
    elif step == "End":
        for j in range(1, 5):
            ws.cell(row=i, column=j).fill = OIL_FILL
            ws.cell(row=i, column=j).font = WHITE_BOLD
    elif i % 2 == 0:
        for j in range(1, 5):
...
chart.y_axis.title = "Cumulative BOPD"; chart.x_axis.title = "Step"
...
for i, opp in enumerate(OPPORTUNITIES, start=5):
    ...
    vals = [opp[0], opp[1], capex_mid, gain_mid, round(ratio, 0), opp[7]]
    for j, v in enumerate(vals, start=1):
        c = ws.cell(row=i, column=j, value=v)
        c.font = BOLD if j == 1 else NORM; c.border = BORDER
        if j in (3, 4, 5): c.alignment = RGT
        if i % 2 == 0: c.fill = ALT_FILL

chart = ScatterChart(); chart.title = "Capex vs Gain — opportunity positioning"
chart.style = 13; chart.x_axis.title = "Capex midpoint ($M)"
...
chart.height = 12; chart.width = 20; chart.legend = None
ws.add_chart(chart, f"A{5 + len(OPPORTUNITIES) + 3}")
...
for opp in OPPORTUNITIES:
    ...
    chart = BarChart(); chart.type = "bar"; chart.title = f"§{opp[0]} gain range (BOPD)"
    sd2 = wb["_chart_data"]
    ...
    sd2.cell(row=base_row+1, column=1, value="2026 low");  sd2.cell(row=base_row+1, column=2, value=opp[3])
    sd2.cell(row=base_row+2, column=1, value="2026 high"); sd2.cell(row=base_row+2, column=2, value=opp[4])
    sd2.cell(row=base_row+3, column=1, value="Full low");  sd2.cell(row=base_row+3, column=2, value=opp[5])
    sd2.cell(row=base_row+4, column=1, value="Full high"); sd2.cell(row=base_row+4, column=2, value=opp[6])
...
agenda = [
    "Validate the bottleneck classification (9 categories) against AON's own internal view",
    "Confirm the FCT findings (once delivered) and align on §4.7 Surface back-pressure priorities",
    "Decide on §4.6 tracer campaign scope — which WI/producer pairs (139-SS, 145-SS first)",
    "Approve the §4.3 VSD retrofit shortlist and 2026 execution sequence",
    "Align on §4.5 Phase 2 Cluster #2 uplift study scope and timing",
    "Cross-reference §4.1 stim backlog with Obj 1.1C selected 5 to avoid double-counting",
    "Workshop output: signed-off opportunity list with executive owners assigned per item",
]
